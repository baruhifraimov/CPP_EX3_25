...
bool Game::check_general_intervention(Player& attacker, Player& target){
	...
	if(generals.size()>0){
		for(Player* general : generals){
			// Skip if the general is the attacker/target
			if(general == &attacker || general == &target){
				continue;
			}

			// Ask general if they want to intervene
			if(ask_general_intervention(*general, attacker, target)){
				return true;
			}
		}
	}
...
bool Game::check_judge_intervention(Player& attacker){
	...
	if(judges.size()>0){
		for(Player* judge : judges){
			...
			// Ask general if they want to intervene
			if(ask_judge_intervention(*judge, attacker)){
				return true;
			}
		}
	}
